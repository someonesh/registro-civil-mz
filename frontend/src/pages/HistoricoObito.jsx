...
export default function HistoricoObito() {
  ...
  useEffect(() => {
    api.get('/obito/historico').then(r => setRegistos(r.data.registos)).catch(() => {})
  }, [])
  ...
  return (
    <div className="p-8" style={{ fontFamily: "'Georgia', serif" }}>
      <div className="mb-6 pb-4 border-b-2 border-[#003F20]">
        <h2 className="text-2xl font-bold text-[#003F20]">Histórico de Óbitos</h2>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded divide-y divide-[#F0F4F8]" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        <div className="grid grid-cols-12 px-5 py-3 bg-[#F7FAFC] border-b border-[#E2E8F0]">
          ...
          <p className="col-span-2 text-xs font-bold text-[#4A5568] uppercase tracking-wide">Causa da Morte</p>
        ...
        {filtrados.map(r => (
          <div key={r.id}
            ...
            <div className="col-span-2">
              <p className="text-xs text-[#4A5568]">{r.confirmado_por || r.rejeitado_por || '—'}</p>
            </div>
