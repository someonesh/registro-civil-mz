...
export default function PendenteObito() {
  const [registos, setRegistos] = useState([])
  const [filtro, setFiltro] = useState('todos')
  const navigate = useNavigate()

  useEffect(() => {
    api.get('/obito/pendentes').then(r => setRegistos(r.data.registos)).catch(()=>{})
  }, [])
  ...
  return (
    <div className="p-8" style={{fontFamily: "'Georgia', serif"}}>
      <div className="mb-6 pb-4 border-b-2 border-[#009A44]">
        <h2 className="text-2xl font-bold text-[#003F20]">Registos de Óbito</h2>
