...
export default function Registados() {
  ...
  useEffect(() => {
    ...
    const p1 = api.get('/nascimento/registados')
      ...
      .catch(e => setErroNasc(`Erro ${e.response?.status || ''}: ${e.response?.data?.detail || e.message}`))

    const p2 = api.get('/obito/registados')
      .then(r => setObitos(r.data.registos || []))
      .catch(e => setErroObit(`Erro ${e.response?.status || ''}: ${e.response?.data?.detail || e.message}`))
  ...
  return (
    <div className="p-8" style={{ fontFamily: "'Georgia', serif" }}>
      ...
      {loading && <p className="text-sm text-[#718096] mb-4">A carregar registos...</p>}

      {erroNasc && (
        <div className="mb-3 p-3 rounded border bg-red-50 border-red-200 text-red-700 text-sm">
          <strong>Nascimentos:</strong> {erroNasc}
        </div>
      )}
      {erroObit && (
        <div className="mb-3 p-3 rounded border bg-red-50 border-red-200 text-red-700 text-sm">
          <strong>Óbitos:</strong> {erroObit}
        </div>
      )}
      ...
      <div className="flex gap-2 mb-5">
        {[
          ['nascimentos', `Nascimentos (${nascimentosFiltrados.length})`],
          ['obitos', `Óbitos (${obitosFiltrados.length})`]
        ].map(([val, label]) => (
      ...
      {tab === 'nascimentos' && (
        <div className="bg-white border border-[#E2E8F0] rounded divide-y divide-[#F0F4F8]" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          {!loading && nascimentosFiltrados.length === 0 && (
      ...
      {tab === 'obitos' && (
        <div className="bg-white border border-[#E2E8F0] rounded divide-y divide-[#F0F4F8]" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          {!loading && obitosFiltrados.length === 0 && (
            <p className="px-5 py-8 text-sm text-[#A0AEC0] text-center">Nenhum registo encontrado</p>
          )}
        </div>
      )}
